...
var nhdID = "1zvE4BWV6DAFSryv40GZ4t3oiL2-pJqM1UThNFgQ"
var NHDLayer = new google.maps.FusionTablesLayer({
  query: {
    select: 'geometry',
    from: nhdID
  },
...
var markersArray = [];
	...
	function drawVisualization() {	  
	  ...
	  function handleQueryResponse(response) {
		...
		// Define a Category control for the '' column
		 ...
		 // Define a numberFilter control for the '' column
			  mileFilter = new google.visualization.ControlWrapper({
				'controlType': 'NumberRangeFilter',
				'containerId': 'mileFilter_div',
				'options': {
				  'filterColumnIndex': 5,
				  //"ui": {'cssClass': 'numSliderCss'},
		...
		*/
		...
		bind(roadFilter, mileFilter).bind(mileFilter, table);
		
		//google.visualization.events.addListener(roadFilter, 'statechange', reFreshMiles);
	...
	function selectHandler() {
		...
		var culLng;
			  ...
			  for (var i = 0; i < selection.length; i++) {
				...
				if (item.row != null) {
				  message += table.getDataTable().getFormattedValue(item.row, 7);
		...
		function showScatter(){
	        ...
	        scatterTable.addRows([
	          [Number(selectTableData.getValue(18,1)), Number(selectTableData.getValue(22,1))]
	        ]);
		...
		function showColumn(){
	        ...
	        columnTable.addRows([
	          [selectTableData.getFormattedValue(18,0), Number(selectTableData.getValue(18,1))],
	          [selectTableData.getFormattedValue(22,0), Number(selectTableData.getValue(22,1))],
	          [selectTableData.getFormattedValue(23,0), Number(selectTableData.getValue(23,1))],
	          [selectTableData.getFormattedValue(27,0), Number(selectTableData.getValue(27,1))],
	          [selectTableData.getFormattedValue(28,0), Number(selectTableData.getValue(28,1))]
	        ]);
	...
	function clickNHD () {
		...
		var cBox = document.getElementById('NHDcheck');
		if(cBox.checked != true){
			NHDLayer.setMap(null);
		}
		else{
			NHDLayer.setMap(map)};		
